...
def get_weather(city_name, date_to_check: datetime.date):
    ...
    if date_to_check is not None and city_name is not None:
        ...
        url = f"https://api.weatherapi.com/v1/forecast.json?q={city_name}&days={days_from_now}&key={api_key}"
        mode = 1
    elif date_to_check is not None and city_name is None:
        days_from_now = (date_to_check - datetime.date.today()).days
        city_name = ""  # fill me in
        mode = 1
        url = f"https://api.weatherapi.com/v1/current.json?q={city_name}&days={days_from_now}&key={api_key}"
    ...
    if response.status_code != 400:
        if mode == 0:
            ...
            new_user_response = f"In {x['location']['name']}, it will be {round(int(x['current']['temp_f']))} degrees Fahrenheit."
